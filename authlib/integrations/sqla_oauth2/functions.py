...
async def create_query_client_func(session, client_model):
    ...
    async def query_client(client_id):
        return await session.scalar(select(client_model).where(client_model.client_id == client_id))
...
def create_revocation_endpoint(session, token_model):
    ...
    :param token_model: Token model class
    """
    from authlib.authlib.oauth2.rfc7009 import RevocationEndpoint
...
def create_bearer_token_validator(session, token_model):
    ...
    :param token_model: Token model class
    """
    from authlib.authlib.oauth2.rfc6750 import BearerTokenValidator
